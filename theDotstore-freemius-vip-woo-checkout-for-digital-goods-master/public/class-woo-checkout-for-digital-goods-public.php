    ...
    public function wcdg_add_quick_checkout_after_add_to_cart_product_page() {
        ...
        if( 'wcdg_down_virtual' === $woo_checkout_unserlize_array['wcdg_chk_on'] ){
            ...
            if ($product-> is_virtual('yes') || $product->is_downloadable('yes') || (isset($is_virtual) && !empty($is_virtual)) || (isset($is_downloadable) && !empty($is_downloadable)) ) {
                ...
                $quick_checkout_text = apply_filters('quick_checkout_text',esc_html__( "Quick Checkout", WCDG_TEXT_DOMAIN));
                echo '<a href="'.esc_url($addtocart_url).'" class="'.esc_attr($button_class).'">'.esc_html__( $quick_checkout_text, WCDG_TEXT_DOMAIN).'</a>';
            }
        }
        if ( wcfdg_fs()->is__premium_only() ) {
            if ( wcfdg_fs()->can_use_premium_code() ) {
                if( 'wcdg_chk_list' === $woo_checkout_unserlize_array['wcdg_chk_on'] ) {
                    ...
                    // check product type is variable or variable-subscription
                    if ( $product->is_type( 'variable' ) || $product->is_type( 'variable-subscription' ) ) {
                        ...
                        if (isset($get_variations) && !empty($get_variations)) {
                            foreach ($get_variations as $get_variation) {
                                $get_variations_id[] = $get_variation['variation_id'];
                            }
                        }
                    }

                    // check current product variation is match with checkout page or not
                    $match_variation = false;
                    if (isset($get_variations_id) && !empty($get_variations_id)) {
                        foreach ( $get_variations_id as $get_variations_id_data ) {
                            if ( 'yes' === get_post_meta($get_variations_id_data,'_wcdg_chk_product',true)) {
                                 $match_variation = true;
                             }
                        }
                    }

                    if( get_post_meta(get_the_ID(),'_wcdg_chk_product',true) || true === $match_variation || ($cat_flag !== 0 ) || ($tag_flag !== 0 ) ){
                        $addtocart_url = wc_get_checkout_url().'?add-to-cart='.$product->get_id();
                        $button_class  = 'single_add_to_cart_button button alt custom-checkout-btn';
        ...
        if( 'wcdg_down_virtual' === $woo_checkout_unserlize_array['wcdg_chk_on'] ){
            ...
            if ($product-> is_virtual('yes') || $product->is_downloadable('yes')) {
                ...
                if( $product->is_type( 'simple' ) || $product->is_type( 'subscription' ) ){
                    ...
                    echo '<a href="'.esc_url($url).'" class="single_add_to_cart_button button alt">'.esc_html__( $quick_checkout_text, WCDG_TEXT_DOMAIN).'</a>';
                }
            } 
        }
        if ( wcfdg_fs()->is__premium_only() ) {
